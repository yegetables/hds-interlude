...
exports.renderLogMessage = renderLogMessage;
exports.detectLogAction = detectLogAction;
exports.formatLayeredLog = formatLayeredLog;
...
function renderLogMessage(message, args = []) {
    return (0, node_util_1.format)(message, ...args.map(value => value instanceof Error ? value.message : value));
}
function detectLogAction(message, level) {
    if (level === 'error')
        return 'error';
    if (/重试|再次尝试/.test(message))
        return 'retry';
    if (/模型调用失败|主叙事失败|消息投递失败/.test(message))
        return 'error';
    if (level === 'warn' || /警告|拦截|不可用|失败/.test(message))
        return 'warning';
    if (/Alter.*(?:触发|超过阈值)|累积触发/.test(message))
        return 'trigger';
    if (/(?:模型调用|情绪偏移生成|记忆整理|后台扫描|剧本推进).*完成/.test(message))
        return 'complete';
    if (/情绪偏移|Alter/.test(message))
        return 'emotion';
    if (/Agency|主动联系判断|主动联系重查/.test(message))
        return 'agency';
    if (/记忆|压缩|Overlay/.test(message))
        return 'memory';
    if (/群消息|群聊|群发言/.test(message))
        return 'group';
    if (/投递|发送/.test(message))
        return 'send';
    if (/收到|接收|入队/.test(message))
        return 'receive';
    if (/模型调用开始|分析开始|读取开始|整理开始/.test(message))
        return 'processing';
    if (/完成|成功|已就绪|已启动/.test(message))
        return 'complete';
    if (/推进|后台扫描/.test(message))
        return 'advance';
    if (/等待|计时器|排队/.test(message))
        return 'waiting';
    return 'system';
}
...
function logCategory(action, phase, standalone = false, message = '') {
    if (action === 'trigger' || action === 'emotion' || /Alter|情绪偏移/.test(message))
        return '[情绪追踪]';
    if (action === 'agency' || /Agency/.test(message))
        return '[主体节奏]';
    if (action === 'memory' || /记忆|压缩|Overlay/.test(message))
        return '[记忆整理]';
    if (action === 'group' || /群聊|群消息/.test(message))
        return '[群聊]';
    if (action === 'retry')
        return '[自动重试]';
    if (standalone)
...
function isRootLog(summary, action, level, standalone) {
    if (standalone || level === 'error' || action === 'error')
        return true;
    if (action === 'trigger' || action === 'memory' && /开始/.test(summary))
        return true;
    if (action === 'advance' && /(?:开始|即将执行)/.test(summary))
        return true;
    if (action === 'receive' && /(?:收到|接收)/.test(summary))
        return true;
    if (action === 'group' && /收到/.test(summary))
        return true;
    return false;
}
function isFinalBranch(summary, action) {
    if (action === 'send')
        return true;
    if (action === 'complete' && !/模型调用完成/.test(summary))
        return true;
...
function categoryColor(action, phase, message, palette) {
    ...
    if (action === 'warning' || action === 'retry')
...
function actionColor(action, palette) {
    ...
    if (action === 'warning' || action === 'retry')
        return palette.warning;
    if (action === 'complete' || action === 'send')
        return palette.success;
    if (action === 'trigger' || action === 'emotion')
        return palette.alter;
    if (action === 'memory' || action === 'advance')
        return palette.memory;
    if (action === 'agency')
        return palette.user;
    return palette.user;
