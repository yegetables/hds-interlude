...
exports.SEMANTIC_STICKER_LIMIT = exports.resolveBlackBoxConfig = exports.InterludeService = void 0;
exports.isHistoryEntryVisibleToParticipant = isHistoryEntryVisibleToParticipant;
exports.shouldRequestTurnEmbedding = shouldRequestTurnEmbedding;
exports.extractSessionVoiceCount = extractSessionVoiceCount;
exports.mergeUserMessageWithVoiceTranscripts = mergeUserMessageWithVoiceTranscripts;
exports.calibratedNativeFaceWillingness = calibratedNativeFaceWillingness;
exports.stableStickerAssetId = stableStickerAssetId;
exports.extractUserReportedTimes = extractUserReportedTimes;
exports.describeQuotedMessage = describeQuotedMessage;
exports.normalizeQuotedMessageContent = normalizeQuotedMessageContent;
exports.normalizeAllowedReactions = normalizeAllowedReactions;
exports.normalizeTimelinePlan = normalizeTimelinePlan;
exports.timelineEntryPromptProjection = timelineEntryPromptProjection;
exports.normalizeGroupChatActions = normalizeGroupChatActions;
exports.formatGroupSpeaker = formatGroupSpeaker;
exports.normalizeGroupVisibleReply = normalizeGroupVisibleReply;
exports.visibleReplyMode = visibleReplyMode;
exports.hasRequiredNarrativeScript = hasRequiredNarrativeScript;
exports.resolveBlindModeConfig = resolveBlindModeConfig;
exports.normalizeScenePresenceDrafts = normalizeScenePresenceDrafts;
exports.groupDueIntents = groupDueIntents;
exports.shouldSupersedeNarrativeRequest = shouldSupersedeNarrativeRequest;
exports.normalizeDatabaseRow = normalizeDatabaseRow;
exports.rankStickerCatalog = rankStickerCatalog;
exports.shouldDownscaleImage = shouldDownscaleImage;
...
const group_willingness_1 = require("./group-willingness");
const qq_face_1 = require("./qq-face");
const schedule_preplan_1 = require("./schedule-preplan");
...
function isHistoryEntryVisibleToParticipant(entry, participantId, shareParticipantDetails) {
    if (shareParticipantDetails)
        return true;
    if (entry.kind === 'group-message' || entry.kind === 'character-group-message')
        return false;
...
function shouldRequestTurnEmbedding(embedding, stickerLibraryEnabled, stickerCount) {
    ...
    return embedding.liveQuery === true
        ...
        || embedding.semanticStickerFilter === true && stickerLibraryEnabled && stickerCount > exports.SEMANTIC_STICKER_LIMIT;
}
...
const SCHEDULE_PREPLAN_RETRY_BACKOFF = 2 * koishi_1.Time.hour;
...
class InterludeService extends koishi_1.Service {
    static { this.inject = ['database', 'http']; }
    constructor(ctx, config) {
        ...
        this.historyVectors = new Map();
        this.historyVectorsReady = new Set();
        /** Per-story retry-after timestamps for failed Schedule Preplan generations. */
        this.schedulePreplanBackoff = new Map();
        this.stickerById = new Map();
        ...
        (0, database_1.registerTables)(ctx);
        ...
        this.narrator = (0, narrator_1.createNarrator)(ctx, config.model, this.blindModeConfig.enabled, onUsage);
        this.compactor = (0, narrator_1.createCompactor)(ctx, config.model, this.blindModeConfig.enabled, onUsage);
        this.embedder = (0, narrator_1.createEmbedder)(ctx, config.model);
        this.stickerDescriber = (0, narrator_1.createStickerDescriber)(ctx, config.model, this.blindModeConfig.enabled, onUsage);
        this.visionDescriber = (0, narrator_1.createVisionDescriber)(ctx, config.model, this.blindModeConfig.enabled, onUsage);
        ...
        this.reportStandaloneOperation('summary', 'info', '服务初始化完成 模型连接=%s 共享主剧本=%s 自动推进=%s', (0, narrator_1.usesRemoteProviders)(config.model) ? '已配置' : '未配置', this.sharedStoryConfig.enabled, this.autoAdvanceConfig.enabled);
    }
    startBackgroundTasks() {
        ...
        this.ctx.setInterval(() => void this.sweep().catch(error => this.reportStandalone('warn', '后台推进失败 错误=%s', error)), sweepInterval * koishi_1.Time.minute);
        if (this.memoryConfig.enabled || this.schedulePreplanConfig.enabled)
            this.ctx.setInterval(() => void this.compactStories().catch(error => this.reportStandalone('warn', '后台整理失败 错误=%s', error)), Math.max(1, this.memoryConfig.backgroundIntervalMinutes) * koishi_1.Time.minute);
        if (this.blindModeConfig.enabled) {
            this.ctx.setInterval(() => this.reportBlindModeHealth(), this.blindModeConfig.healthReportMinutes * koishi_1.Time.minute);
        }
        if (this.stickerConfig.enabled) {
            this.ctx.setTimeout(() => void this.scanStickerLibrary(), 0);
            this.ctx.setInterval(() => void this.scanStickerLibrary(), 5 * koishi_1.Time.minute);
        }
    ...
    setNarrator(provider) { this.narrator = provider; }
    getNarrator() { return this.narrator; }
    setCompactor(provider) { this.compactor = provider; }
    /** Allows a custom/local vector service without replacing the main narrator. */
    setEmbedder(provider) { this.embedder = provider; }
    /**
    ...
    canHandleSession(session) {
        if (!isOneBotPlatform(session.platform))
            return true;
        ...
        if (!allowed)
            this.reportStandaloneOperation('diagnostic', 'debug', 'OneBot 白名单拒绝用户账号 原始用户ID=%s 规范化ID=%s', session.userId, userId);
        return allowed;
    }
    ...
    canHandleParticipant(participant) {
        if (!isOneBotPlatform(participant.platform))
            return true;
        const config = this.config.onebot;
        if (!config?.enabled)
            return true;
        if (!isEnabledAccount(config.botAccounts, normalizeAccountId(participant.selfId)))
            return false;
    ...
    canHandleStory(story) {
        if (!isOneBotPlatform(story.platform))
            return true;
        const config = this.config.onebot;
        if (!config?.enabled)
            return true;
    ...
    async findStory(session) {
        if (this.sharedStoryConfig.enabled) {
            ...
            if (existing) {
                ...
                if (existing.platform === session.platform && existing.id !== sharedId)
                    return this.migrateLegacyStory(existing, session);
    ...
    async createStory(session, name) {
        if (!this.canHandleSession(session) && !this.canHandleGroupSession(session))
            throw new Error('This session is not allowed to use HDS Interlude.');
        ...
        const story = {
            ...
            channelId: '', status: 'active', setting, state: (0, types_1.emptyStoryState)(),
    ...
    async storyStartReadiness(session) {
        ...
        const warnings = [];
        if (!this.canHandleSession(session))
            blockers.push('当前机器人账号或用户账号未通过 OneBot 白名单。');
        if (!setting.character.name.trim())
            blockers.push('storyDefaults.characterName 为空。');
        if (!setting.character.profile.trim())
            blockers.push('storyDefaults.characterProfile 尚未填写。');
        try {
        ...
        if ((0, narrator_1.usesRemoteProviders)(this.config.model)) {
            const providers = (0, narrator_1.configuredProviders)(this.config.model);
            const assigned = providers.find(item => item.enabled && item.endpoint && item.model && item.useForMain === true);
            const mainModelId = (0, narrator_1.effectiveMainModelId)(this.config.model);
            ...
            if (!provider)
                blockers.push('没有可用的主叙事模型：请在模型中心勾选一条“用作主叙事模型”。');
        }
        else {
            warnings.push('尚未配置启用的模型连接：可用于安装验证，但不会生成远程叙事。');
        }
        if (!setting.perspective.trim())
            warnings.push('Perspective 尚未填写；主角将仅使用 Canon 与已有 Overlay。');
        if (!setting.world.trim())
            warnings.push('world 尚未填写；建议在 Console 补充现实边界与地点背景。');
        const existing = await this.findStory(session);
        return {
     ...
     */
    async ensureParticipant(story, session, now = new Date(), knownExisting) {
        ...
        const participant = {
            ...
            state: (0, types_1.emptyParticipantState)(), status: 'active', createdAt: now, updatedAt: now,
        };
        try {
    ...
    async recentEntriesForPrompt(storyId, now) {
        ...
        const [countRows, timeRows] = await Promise.all([
            ...
            minutes > 0
                ? this.dbGet('interlude_script_entry', { storyId, occurredAt: { $gte: new Date(now.getTime() - minutes * koishi_1.Time.minute) } }, { limit: 500, sort: { occurredAt: 'desc' } })
                : Promise.resolve([]),
        ]);
        const byId = new Map();
        for (const entry of [...countRows, ...timeRows])
        ...
        return this.serial(story.id, async () => {
            ...
            if (activeScene) {
                await this.dbSet('interlude_scene', { id: activeScene.id }, {
                    hook: `Host timeline rebased at ${(0, time_1.formatLogTime)(now, current.setting.timezone)}.`,
    ...
    async clearSettingOverlayUnlocked(story, target) {
        ...
        if (target === 'perspective' || target === 'all')
            delete overlay.perspective;
        if (target === 'relationship' || target === 'all')
            delete overlay.relationship;
        if (target === 'world' || target === 'all')
            delete overlay.world;
        ...
        for (const patch of patches) {
            if (!['proposed', 'applied', 'compacted'].includes(patch.status) || (target !== 'all' && patch.target !== target))
                continue;
        ...
        for (const snapshot of snapshots) {
            if (target !== 'all' && snapshot.target !== target)
                continue;
    ...
    async purgeAllStoryData(storyId) {
        ...
        await this.dbSet('interlude_story', { id: storyId }, {
            setting, state: (0, types_1.emptyStoryState)(), cursorAt: now, updatedAt: now,
        });
        await this.resetParticipantCanon(storyId, now);
        await this.ensureContinuity({ ...story, setting, state: (0, types_1.emptyStoryState)(), cursorAt: now }, now);
    }
    ...
    async clearDatabase() {
        if (this.databaseResetting)
            throw new Error('HDSI 数据库清空已经在进行中。');
            ...
            for (const table of tables) {
                ...
                try {
                    ...
                    for (const row of rows) {
                        ...
                        const fallback = table === 'interlude_story'
                            ? { status: 'archived', setting: this.initialStorySetting(), state: (0, types_1.emptyStoryState)() }
                            : table === 'interlude_participant'
                                ? { status: 'paused', profile: '', relationship: '', state: (0, types_1.emptyParticipantState)() }
    ...
    async purgeStoryRange(storyId, from, to) {
        ...
        for (const entry of entries)
            if (entryIds.has(entry.id))
                await this.purgeTable('interlude_script_entry', { id: entry.id }, {
                    kind: 'redacted', actor: 'system', content: '[管理员已删除剧本内容]', metadata: { redacted: true },
                });
        ...
        for (const scene of scenes) {
            ...
            if (overlaps)
                await this.purgeTable('interlude_scene', { id: scene.id }, { status: 'closed', hook: '', summary: '', entryCount: 0 });
        }
        const arcs = await this.dbGet('interlude_arc', { storyId });
        for (const arc of arcs)
            if (inRange(arc.createdAt) || inRange(arc.updatedAt))
                await this.purgeTable('interlude_arc', { id: arc.id }, { status: 'closed', summary: '', sceneCount: 0 });
        const patches = await this.dbGet('interlude_state_patch', { storyId });
        for (const patch of patches)
            if (inRange(patch.createdAt) || inRange(patch.appliedAt))
                await this.purgeTable('interlude_state_patch', { id: patch.id }, { status: 'rejected', proposedValue: '[管理员已删除提案]', evidence: '' });
    ...
    async receiveGroup(session) {
        if (this.databaseResetting || !this.canHandleGroupSession(session))
            return false;
        ...
        if (rule.responseMode === 'mention-only' && !mentionedBot)
            return false;
        let story = await this.findStory(session);
        if (!story && this.config.runtime.autoCreate)
            story = await this.createStory(session);
        ...
        const messageContent = (0, qq_face_1.normalizeQQNativeFaceSegments)(session.content);
    ...
    async receive(session) {
        ...
        const accepted = await this.serial(story.id, async () => {
            ...
            const superseded = await this.cancelPendingOutgoingMessages(current.id, incomingParticipant.id, now, this.config.runtime.cancelDelayedRepliesOnUserMessage);
            await this.appendEntry(current.id, {
    ...
    async lookupGroupMemberName(cacheKey, groupId, userId, selfId) {
        const bot = this.ctx.bots.find(item => String(item.selfId) === String(selfId)
            && (item.platform === 'onebot' || isOneBotPlatform(item.platform)));
        ...
        try {
            ...
            if (!name)
                return '';
            this.groupMemberNameCache.set(cacheKey, { name, expiresAt: Date.now() + 12 * koishi_1.Time.hour });
            return name;
        }
        catch {
            return '';
    ...
    bufferGroupMessage(story, rule, session, message, mentionedBot, quotedBot) {
        ...
        const delay = Math.max(0, rule.debounceSeconds ?? 1) * koishi_1.Time.second;
    ...
    async flushGroupTurn(key, revision) {
        ...
        if (!turn || turn.revision !== revision || this.databaseResetting)
            return;
        ...
        try {
            ...
            if (!turn.messages.length && !turn.timer)
                this.bufferedGroupTurns.delete(key);
            return;
        }
        ...
        const willingness = (0, group_willingness_1.evaluateGroupWillingness)(this.groupWillingness.get(key), turn.rule.willingness, {
        ...
        if (!willingness.shouldCall) {
            this.reportOperation('diagnostic', 'debug', story, 'user-message', '群聊意愿未触发模型调用 群=%s 分数=%s 概率=%s 原因=%s', turn.groupId, willingness.state.score.toFixed(3), willingness.probability.toFixed(3), willingness.reason);
            if (!turn.messages.length && !turn.timer)
                this.bufferedGroupTurns.delete(key);
            return;
        }
        ...
        try {
            ...
            const result = await this.serial(story.id, async () => {
                if (this.databaseResetting || !succeeded)
                    return { content: '', messages: [], chatActions: { reactions: [] } };
                const current = await this.getStory(story.id);
                ...
                if (succeeded)
                    await this.scheduleConversationFollowUpsAfterTurn(current.id, snapshot.now, decision.interaction);
            ...
            const completedReactions = result.chatActions.reactions.length && turn.latestSession
                ...
                : 0;
            const groupDelivery = result.content
            ...
            if (groupDelivery.deliveredSegments.length || completedReactions || stickerDelivered || nativeFaceDelivered) {
                this.groupWillingness.set(key, (0, group_willingness_1.consumeGroupWillingness)(this.groupWillingness.get(key), turn.rule.willingness, Date.now()));
            }
            this.scheduleCompaction(story.id);
        }
        catch (error) {
    ...
    async groupMessages(storyId, groupId, limit) {
        ...
        return rows
            ...
            speaker: formatGroupSpeaker(String(entry.metadata?.senderName ?? (entry.actor === 'character' ? '主角' : entry.metadata?.senderId ?? '群成员')), String(entry.metadata?.senderId ?? (entry.actor === 'character' ? 'character' : 'unknown'))),
    ...
    async groupCooldownActive(storyId, groupId, cooldownSeconds) {
        ...
        return !!latest && Date.now() - latest.occurredAt.getTime() < cooldownSeconds * koishi_1.Time.second;
    }
    groupChatCapabilities(session, messages) {
        const config = this.config.chatActions;
        if (!config?.enabled || !session || !isOneBotPlatform(session.platform) || !config.platforms?.includes('qq'))
            return undefined;
        if (!messages.some(message => !!message.messageRef && !!message.messageId))
            return undefined;
        ...
        if (!quoteReply && !reactions.length && !nativeFaces.length)
            return undefined;
    ...
    resolveSticker(draft, catalog) {
        if (!draft || typeof draft.assetId !== 'string' || typeof draft.willingness !== 'number' || !catalog.some(item => item.assetId === draft.assetId))
            return undefined;
        if (normalizeExpressionThreshold(draft.willingness) < this.expressionThreshold)
            return undefined;
    ...
    async sendSticker(story, session, channelId, asset, groupId) {
        const root = (0, node_path_1.resolve)(this.ctx.baseDir, this.stickerConfig.directory);
        const file = (0, node_path_1.resolve)(root, asset.filePath);
        const relativePath = (0, node_path_1.relative)(root, file);
        if (!relativePath || relativePath === '..' || relativePath.startsWith(`..${node_path_1.sep}`) || relativePath.includes(':'))
            return false;
        try {
            await session.bot.sendMessage(channelId, (0, koishi_1.h)('img', { src: (0, node_url_1.pathToFileURL)(file).href }));
            const now = new Date();
            await this.serial(story.id, async () => this.appendEntry(story.id, {
                kind: 'character-platform-action', actor: 'character',
    ...
    async sendNativeFace(story, session, channelId, semantic, groupId) {
        try {
            await session.bot.sendMessage(channelId, (0, koishi_1.h)('face', { id: QQ_NATIVE_FACE_IDS[semantic] }));
            const now = new Date();
            await this.serial(story.id, async () => this.appendEntry(story.id, {
    ...
    async sendGroupMessage(story, channelId, content, replyToMessageId, session) {
        ...
        const sessionBot = session?.bot && typeof session.bot.sendMessage === 'function'
            ? session.bot
            : undefined;
        ...
        for (const [index, segment] of this.splitOutgoingMessage(content).entries()) {
            const outgoing = index === 0 && replyToMessageId
                ? [(0, koishi_1.h)('quote', { id: replyToMessageId }), segment]
                : segment;
            try {
    ...
    bufferUserNarrative(story, participant, session, now, supersededIntents, content = String(session.content ?? ''), imageSources = [], quote) {
        ...
        const delay = Math.max(0, this.config.runtime.userMessageDebounceSeconds ?? 2) * koishi_1.Time.second;
    ...
    signalIncomingInterruption(story, participant) {
        ...
        if (!turn || !shouldSupersedeNarrativeRequest(turn.inFlightRequestId, turn.firstMessageCommittedRequestId, turn.obsoleteRequestIds))
            return;
        turn.obsoleteRequestIds.add(turn.inFlightRequestId);
        this.reportOperation('standard', 'info', story, 'user-message', '新消息到达且首条回复尚未提交，放弃旧请求 参与者=%s 请求=%d', participant.id, turn.inFlightRequestId);
    }
    ...
    async deliverEarlyPrivateReply(story, participant, session, turn, requestId, reply) {
        if (reply.kind !== 'private' || !reply.interaction || reply.interaction.reply.mode !== 'immediate')
            return false;
        if (turn.nextRevision !== requestId || turn.obsoleteRequestIds.has(requestId) || turn.firstMessageCommittedRequestId === requestId)
            return false;
        if (!this.canHandleParticipant(participant))
            return false;
        const content = reply.interaction.reply.content?.trim() || '';
        if (!content || this.splitOutgoingMessage(content).length !== 1)
            return false;
    ...
    get voiceTranscriptionConfig() {
        if (this.cachedVoiceTranscriptionConfig)
    ...
    async scanStickerLibrary() {
        ...
        if (!config.enabled || this.stickerScanRunning)
            return;
        this.stickerScanRunning = true;
        try {
            const root = (0, node_path_1.resolve)(this.ctx.baseDir, config.directory);
            ...
            for (const file of files) {
                const filePath = (0, node_path_1.relative)(root, file).replace(/\\/g, '/');
                ...
                try {
                    ...
                    if (prior?.hash === hash && prior.status === 'active')
                        continue;
                    ...
                    const base = {
                        ...
                        size: bytes.length, hash, description: '', aliases: [], status: 'pending', updatedAt: now,
                    };
                    const asset = prior
                        ? await this.dbSet('interlude_sticker', { id: prior.id }, base).then(() => ({ ...prior, ...base }))
                        : await this.dbCreate('interlude_sticker', { ...base, createdAt: now });
            ...
            for (const asset of existing) {
                if (asset.status !== 'missing' && !seen.has(asset.filePath))
                    await this.dbSet('interlude_sticker', { id: asset.id }, { status: 'missing', updatedAt: new Date() });
            }
            ...
            for (const item of pending.slice(0, 5)) {
                if (!this.stickerDescriber.available())
                    break;
                ...
                if (updated && this.semanticStickerEmbeddingEnabled()) {
                    ...
                    if (embedding.length)
                        await this.dbSet('interlude_sticker', { id: item.asset.id }, { embedding, updatedAt: new Date() });
                }
    ...
    async backfillStickerEmbeddings() {
        if (!this.semanticStickerEmbeddingEnabled())
            return;
        ...
        for (const asset of pending) {
            ...
            if (embedding.length)
                await this.dbSet('interlude_sticker', { id: asset.id }, { embedding, updatedAt: new Date() });
        }
    }
    async stickerCatalogForSession(session, turnQueryEmbedding) {
        const config = this.stickerConfig;
        if (!config.enabled || !session || !isOneBotPlatform(session.platform))
            return [];
    ...
    async rankStickerAssets(turnQueryEmbedding) {
        ...
        if (!this.config.model.embedding?.semanticStickerFilter)
            return assets;
        if (!turnQueryEmbedding?.length || assets.length <= exports.SEMANTIC_STICKER_LIMIT)
            return assets;
        return rankStickerCatalog(assets, turnQueryEmbedding, exports.SEMANTIC_STICKER_LIMIT);
    }
    semanticTurnEmbeddingEnabled() {
        return shouldRequestTurnEmbedding(this.config.model.embedding, this.stickerConfig.enabled, this.stickerCatalog.length);
    }
    ...
    async previousSceneSummaries(storyId) {
        ...
        if (!limit || !this.memoryConfig.enabled)
            return [];
    ...
    async recallHistory(storyId, participantId, turnQueryEmbedding, excludeIds) {
        ...
        for (const [id, item] of cache) {
            if (excludeIds.has(id))
                continue;
            if (!isHistoryEntryVisibleToParticipant(item, participantId, this.sharedStoryConfig.shareParticipantDetails))
                continue;
    ...
    async ensureHistoryVectors(storyId) {
        if (this.historyVectorsReady.has(storyId))
            return;
        ...
        try {
            ...
            for (const row of rows) {
                if (!row.embedding?.length)
                    continue;
                if (!RECALLABLE_ENTRY_KINDS.includes(row.kind))
                    continue;
                cache.set(row.id, {
                    vector: row.embedding,
                    content: (0, narrator_1.promptVisibleMessageContent)(row.content, (0, narrator_1.recentScriptOwnership)(row)),
    ...
    async transcribeVoiceEvent(story, session) {
        ...
        try {
            const response = await withTimeout(Promise.resolve(internal._request('fetch_ptt_text', { message_id: messageId })), config.timeoutMs);
            if (response?.retcode != null && Number(response.retcode) !== 0)
                throw new Error(String(response?.wording || response?.message || `retcode=${response.retcode}`));
            if (response?.status && response.status !== 'ok')
                throw new Error(String(response?.wording || response?.message || response.status));
            const data = response?.data ?? response;
            const text = typeof data?.text === 'string' ? clip(data.text, 4_000) : '';
            if (!text)
                throw new Error('SnowLuma returned an empty transcription');
    ...
    describeVisionEvent(session) {
        ...
        const text = (0, qq_face_1.normalizeQQNativeFaceSegments)(raw)
    ...
    async loadNativeImages(story, sources, session) {
        if (!this.config.model.vision?.enabled || !sources.length)
            return [];
        const images = [];
        for (const [index, source] of sources.slice(0, 3).entries()) {
            try {
                ...
                if (image)
                    images.push({ id: `turn-image-${index + 1}`, ...image });
            }
            catch (error) {
    ...
    async fetchNativeImage(source, bot, adapterProvided = false) {
        ...
        if (value.startsWith('onebot-file:')) {
            ...
            for (const candidate of candidates) {
                if (/^https?:\/\//i.test(candidate)) {
                    ...
                    try {
                        const bytes = await (0, promises_1.readFile)(candidate);
        ...
        if (url.protocol !== 'http:' && url.protocol !== 'https:')
            return undefined;
        if (!adapterProvided && !isTrustedImageHost(url.hostname, this.trustedImageHosts))
            return undefined;
    ...
    async downscaleImageForVision(image) {
        ...
        if (!maxDimension || !shouldDownscaleImage(image.mimeType, image.dataUri))
            return undefined;
    ...
    invalidateBufferedNarratives(storyId) {
        for (const [key, turn] of this.bufferedNarrativeTurns) {
            if (storyId && turn.storyId !== storyId)
                continue;
            if (turn.timer)
                turn.timer();
            if (turn.inFlightRequestId)
                turn.obsoleteRequestIds.add(turn.inFlightRequestId);
        ...
        for (const key of this.groupWillingness.keys()) {
            if (!storyId || key.startsWith(`${storyId}:`))
                this.groupWillingness.delete(key);
        }
        for (const [key, wake] of this.dueIntentWakeTimers) {
            if (storyId && key !== storyId)
                continue;
            wake.cancel();
    ...
    hasPendingNarrative(storyId) {
        ...
        for (const turn of this.bufferedNarrativeTurns.values()) {
            if (turn.storyId === storyId && (turn.messages.length || turn.timer || turn.inFlightRequestId))
                return true;
        }
        for (const turn of this.bufferedGroupTurns.values()) {
            if (turn.storyId === storyId && (turn.messages.length || turn.timer))
                return true;
        }
        return false;
    }
    async flushBufferedNarrative(key, revision) {
        ...
        if (!turn || turn.nextRevision !== revision)
            return;
        ...
        try {
            ...
            const snapshot = await this.serial(turn.storyId, async () => {
                ...
                if (!participant || participant.status !== 'active' || story.status !== 'active')
                    return undefined;
                const now = new Date();
            ...
            const turnQueryEmbedding = userMessage?.trim() && this.semanticTurnEmbeddingEnabled()
                ...
                : undefined;
            const quotedMessages = batch.flatMap((message, index) => message.quote
            ...
            const early = { delivered: false, interaction: undefined };
            const narrative = await this.tryDecide(snapshot.story, snapshot.participant, 'user-message', snapshot.from, snapshot.now, userMessage, snapshot.due, superseded, undefined, images, chatCapabilities, quotedMessages, stickerCatalog, turnQueryEmbedding, visualObservations, async (reply) => {
                if (early.delivered)
                    return false;
            ...
            const result = await this.serial(turn.storyId, async () => {
                ...
                if (turn.obsoleteRequestIds.has(requestId))
                    return { obsolete: true, requeue: true, messages: [] };
                ...
                for (const observation of immediateObservations)
                    await this.persistCollectedWebObservation(observation);
                ...
                if (succeeded) {
                    ...
                    if (snapshot.due.length)
                        await this.dbSet('interlude_intent', { id: { $in: snapshot.due.map(intent => intent.id) } }, { status: 'completed', updatedAt: now });
                }
                else {
                    await this.scheduleNarrativeRetry(current.id, currentParticipant.id, now);
                }
                if (succeeded)
                    await this.scheduleConversationFollowUpsAfterTurn(current.id, effectiveNow, decision.interaction, currentParticipant.id);
            ...
            if (this.canHandleParticipant(snapshot.participant)) {
                ...
                if (sticker && turn.latestSession)
                    await this.sendSticker(snapshot.story, turn.latestSession, snapshot.participant.channelId, sticker);
                const nativeFace = sticker ? undefined : this.resolveNativeFace(decision, chatCapabilities);
                if (nativeFace && turn.latestSession)
                    await this.sendNativeFace(snapshot.story, turn.latestSession, snapshot.participant.channelId, nativeFace);
            }
            ...
            if (!turn.messages.length && !turn.timer && !turn.inFlightRequestId)
                this.bufferedNarrativeTurns.delete(key);
        }
    }
    async advanceStory(story, force = true) {
        if (!this.canHandleStory(story))
            return [];
        ...
        if (force || messages.length)
            this.reportOperation('summary', 'info', story, 'advance', '剧本推进完成 可见消息=%d', messages.length);
        this.scheduleCompaction(story.id);
        return messages;
    }
    ...
    async sweep() {
        if (this.databaseResetting || this.sweepRunning)
            return;
        ...
        try {
            ...
            this.reportOperation('diagnostic', 'debug', story, 'advance', '后台扫描开始 游标=%s 下次自动推进=%s', (0, time_1.formatLogTime)(story.cursorAt, story.setting.timezone), (0, time_1.formatLogTime)(toDate(story.state.automation?.nextAdvanceAt), story.setting.timezone));
    ...
    async advanceUnlocked(story, now, force) {
        ...
        for (const intent of splitSegments) {
            ...
            const delivered = await this.sendOutgoingMessages(story, [{ participantId: participant.id, content, automaticDelivery }], undefined, undefined, target => this.interruptedTypingParticipants.has(target.id), false);
            if (!delivered.length) {
                if (this.interruptedTypingParticipants.has(participant.id))
                    continue;
                const retryAt = new Date(now.getTime() + 30 * koishi_1.Time.second);
        ...
        if (splitHandled)
            await this.scheduleNextSplitWake(story.id);
        ...
        for (const intent of browserIntents)
            await this.executeDeferredBrowserIntent(story, intent, now);
        ...
        this.reportOperation('diagnostic', 'debug', story, 'advance', '后台状态 到期计划=%d 分段消息=%d 网页任务=%d 短期跟进=%d 自动推进到期=%s 对话暂停=%s', due.length, splitSegments.length, browserIntents.length, dueFollowUps.length, automaticDue, pausedForConversation);
        ...
        if (!force && !due.length && (!automaticDue || pausedForConversation))
            return messages;
        ...
        const minimumManualAdvanceMs = Math.max(1, this.config.runtime.minimumAdvanceMinutes) * koishi_1.Time.minute;
        const manualAdvanceTooSoon = force
            && !due.length
            && !dueFollowUps.length
            && elapsed < minimumManualAdvanceMs;
        if (manualAdvanceTooSoon) {
            this.reportOperation('standard', 'info', story, 'advance', '手动推进跳过：游标距离现在不足 %d 分钟，且没有到期计划或对话后续任务', this.config.runtime.minimumAdvanceMinutes);
            return messages;
        }
        let advanced = false;
        ...
        if (elapsed > 0 && !hasNarrativeDue && (force || (automaticDue && !pausedForConversation))) {
            ...
            const phase = followUpParticipant?.status === 'active'
                ? 'conversation-follow-up'
                : 'advance';
            this.reportOperation('standard', 'info', story, phase, '即将执行自动写作 类型=%s 时间段=%s→%s', (0, logging_1.phaseLabel)(phase), (0, time_1.formatLogTime)(from, story.setting.timezone), (0, time_1.formatLogTime)(now, story.setting.timezone));
        ...
        if (dueBatch) {
            ...
            this.reportOperation('standard', 'info', current, 'intent-due', '即将处理到期计划 数量=%d 类型=%s 参与者=%s', dueBatch.length, Array.from(new Set(dueBatch.map(intent => intent.type))).join(','), dueParticipant?.id || '全局');
            ...
            if (turnSucceeded) {
                ...
                if (ordinaryDueIds.length)
                    await this.dbSet('interlude_intent', { id: { $in: ordinaryDueIds } }, { status: 'completed', updatedAt: now });
                ...
                if (retries.length) {
                    ...
                    if (streamRecovery)
                        await this.scheduleStreamScriptRecovery(current.id, dueParticipant?.id ?? '', now, attempts);
                    else
        ...
        if (dueBatches.length > 1) {
            const current = await this.getStory(story.id);
            this.reportOperation('standard', 'info', current, 'intent-due', '其余 %d 组到期计划已保留，下一次扫描将按新的时间段继续处理', dueBatches.length - 1);
            this.scheduleDueIntentWake(story.id, new Date(now.getTime() + Math.max(koishi_1.Time.second, this.config.runtime.sweepIntervalMinutes * koishi_1.Time.minute)));
        }
    ...
    async decide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], extraWebContext = [], outputRecovery = false, chatCapabilities, quotedMessages = [], stickerCatalog = [], turnQueryEmbedding, visualObservations, timelinePlan, onEarlyReply) {
        ...
        const [recentEntries, memories, scene, arc, previousScenes, facts, allParticipants, webContext, activeConsequences, overlaySnapshots, followUpCommitments, scheduleRecord, upcomingIntents] = await Promise.all([
            ...
            this.activeConsequencesAndExpire(story.id, now, phase === 'advance' || this.sharedStoryConfig.shareParticipantDetails ? undefined : participant?.id),
        ...
        const visibleEntries = this.sharedStoryConfig.shareParticipantDetails
            ...
            : recentEntries.filter(entry => {
                ...
                if (!groupContext && (entry.kind === 'group-message' || entry.kind === 'character-group-message'))
                    return false;
        ...
        return this.narrator.decide({
            ...
            recentProtectionSince: (this.config.runtime.contextTimeWindowMinutes ?? 60) > 0
                ? new Date(now.getTime() - Math.min(this.config.runtime.contextTimeWindowMinutes ?? 60, 1_440) * koishi_1.Time.minute)
                : undefined,
            ...
            agencyEnabled,
            agencyWindow: agencyEnabled ? (0, agency_1.activeAgencyWindow)(story.state.agencyWindow, now) ?? null : null,
            automaticDeliverySummaries: isAutomaticNarrativePhase(phase)
                ? normalizeStoryState(story.state).automaticDeliverySummaries
                : [],
            followUpCommitments,
            schedulePreplan: (0, schedule_preplan_1.schedulePreplanWindow)(scheduleRecord, now, story.setting.timezone, 12, this.schedulePreplanConfig),
            onEarlyReply,
        });
    }
    ...
    shouldRefreshContinuity(story, phase) {
        ...
        if (state.continuityDirty)
            return true;
        if (phase === 'advance' && !state.continuitySnapshot)
            return true;
    ...
    async planAutomaticTimeline(story, participant, phase, from, now, dueIntents) {
        if (!this.compactor.planTimeline)
            return undefined;
        ...
        const request = {
            ...
            dueIntents, schedulePreplan: (0, schedule_preplan_1.schedulePreplanWindow)(scheduleRecord, now, story.setting.timezone, 12, this.schedulePreplanConfig),
        };
        try {
    ...
    async tryDecide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], chatCapabilities, quotedMessages = [], stickerCatalog = [], turnQueryEmbedding, visualObservations, onEarlyReply) {
        ...
        const startedAt = Date.now();
        this.reportOperation('standard', 'info', story, phase, '模型调用开始 任务=主叙事 模型=%s 参与者=%s 时间段=%s→%s 到期计划=%d', this.mainModelLabel(), participant?.id || '全局', (0, time_1.formatLogTime)(from, story.setting.timezone), (0, time_1.formatLogTime)(now, story.setting.timezone), dueIntents.length);
        try {
            ...
            if ((0, narrator_1.usesRemoteProviders)(this.config.model) && !earlyReplyCommitted && requiresVisibleReplyRecovery(phase, groupContext, decision)) {
                this.reportOperation('standard', 'warn', story, phase, '结构化可见回复缺失，已抛弃本次未落库剧本并重新写作');
            ...
            if ((0, narrator_1.usesRemoteProviders)(this.config.model) && !hasRequiredNarrativeScript(decision)) {
            ...
            this.reportOperation('standard', 'info', story, phase, '模型调用完成 任务=主叙事 耗时=%dms 剧本文字=%d 回复模式=%s', Date.now() - startedAt, result.decision.script?.length ?? 0, visibleReplyMode(result.decision, phase, groupContext));
            return result;
        }
        catch (error) {
    ...
    async persistDecision(story, participant, raw, from, now, permitMessages, phase, contextIntents = [], immediateReplyAlreadyDelivered = false, timelinePlan) {
        ...
        const refreshContinuity = this.shouldRefreshContinuity(story, phase);
        const decision = normalizeDecision(raw, from, now, permitMessages, this.config.runtime, this.sharedStoryConfig, participant?.id ?? '', permittedParticipantIds, phase, this.memoryConfig, refreshContinuity);
        let scriptEntry;
        if (decision.script) {
        ...
        for (const memory of decision.memories)
            await this.appendMemory(story.id, memory, now, memory.participantId ?? participant?.id ?? '');
        ...
        if (phase === 'user-message' && participant) {
            ...
            if (commitment)
                await this.appendFollowUpCommitment(story, participant.id, commitment, scriptEntry?.id, now);
        }
        ...
        if (participant && decision.statePatch)
            await this.updateParticipantState(participant, decision.statePatch, now);
        ...
        if (decision.script) {
            ...
            if (this.agencyConfig.enabled && (phase === 'advance' || isAgencyCheck)) {
                ...
                if (scriptEntry?.id)
                    validSourceEntryIds.add(scriptEntry.id);
                const agencyWindow = (0, agency_1.normalizeAgencyWindowDraft)(decision.agencyWindow, now, this.agencyConfig, validSourceEntryIds, scriptEntry?.id) ?? (0, agency_1.activeAgencyWindow)(state.agencyWindow, now);
                nextState.agencyWindow = agencyWindow;
                agencyCandidate = (0, agency_1.normalizeProactiveContact)(decision.proactiveContact, now, this.agencyConfig, permittedParticipantIds, validSourceEntryIds, scriptEntry?.id);
                if (isAgencyCheck && agencyCandidate?.participantId !== participant?.id)
                ...
                if (agencyCandidate && agencyWindow) {
                    ...
                    const capacity = (0, agency_1.evaluateAgencyCapacity)(agencyWindow, agencyCandidate, now, this.agencyConfig, target?.state.lastCharacterMessageAt);
                    ...
                    if (!agencyAllowsSend && agencyCandidate.outcome !== 'let-go' && willingnessPasses) {
                        agencyRecheck = {
                            ...
                            at: (0, agency_1.proactiveRecheckAt)(agencyCandidate, capacity, agencyWindow, now),
                        };
                    }
                    this.reportOperation('standard', 'info', story, phase, 'Agency 主动联系判断 参与者=%s 结果=%s 原因=%s 意愿=%s', agencyCandidate.participantId, agencyAllowsSend ? '立即联系' : agencyRecheck ? '稍后重查' : '自然放下', capacity.reason, willingness.toFixed(2));
                }
                if (agencyWindow) {
                    this.reportOperation('diagnostic', 'debug', story, phase, 'Agency Window 更新 负荷=%s 隐私=%s 设备=%s 有效至=%s', agencyWindow.activityLoad, agencyWindow.privacy, agencyWindow.deviceAccess, (0, time_1.formatLogTime)(toDate(agencyWindow.validUntil), story.setting.timezone));
                }
            }
            await this.dbSet('interlude_story', { id: story.id }, { state: nextState, updatedAt: now });
            if (alterTurn?.thresholdReached)
                this.scheduleAlterAnalysis(story.id, phase, participant?.id);
        }
        if (agencyRecheck) {
        ...
        const automaticDelivery = isAutomaticNarrativePhase(phase) && scriptEntry
            ...
            : undefined;
        if (participant && !isAgencyCheck && interaction?.seen)
            await this.markParticipantSeen(participant, now);
        ...
        if (phase === 'advance' && decision.crossConversationActions.length && !crossActions.length) {
            this.reportOperation('diagnostic', 'debug', story, phase, 'Agency 拒绝未通过容量或来源验证的 crossConversationAction 数量=%d', decision.crossConversationActions.length);
        }
        ...
        for (const action of crossActions) {
            if (action.mode === 'immediate') {
                ...
                if (action.mode !== 'delayed' || !sendAtValue)
                    continue;
    ...
    get alterSystemConfig() {
        return this.cachedAlterSystemConfig ??= (0, alter_1.resolveAlterSystemConfig)(this.config.alterSystem);
    }
    get agencyConfig() {
        return this.cachedAgencyConfig ??= (0, agency_1.resolveAgencyConfig)(this.config.agency);
    }
    get schedulePreplanConfig() {
        return this.cachedSchedulePreplanConfig ??= (0, schedule_preplan_1.resolveSchedulePreplanConfig)(this.config.schedulePreplan);
    }
    get blindModeConfig() {
    ...
    emotionalOffsetForPrompt(story) {
        return (0, alter_1.emotionalOffsetForPrompt)((0, alter_1.normalizeAlterSystemState)(story.state.alterSystem), this.alterSystemConfig);
    }
    updateAlterSystem(story, current, alter, phase, now) {
        const config = this.alterSystemConfig;
        if (!config.enabled || alter === undefined)
            return undefined;
        const result = (0, alter_1.advanceAlterSystem)(current, alter, phase, now, config);
        if (result.offsetExpired)
            this.reportOperation('standard', 'info', story, phase, 'Alter 情绪偏移已自然消退');
        this.reportOperation('diagnostic', 'debug', story, phase, 'Alter 状态已更新 本轮=%s 累计=%s 阈值=%s 权重=%s', alter, result.state.alterValue, result.threshold.toFixed(2), result.state.alterWeight.toFixed(2));
        return result;
    }
    scheduleAlterAnalysis(storyId, phase, participantId = '') {
        if (this.scheduledAlterAnalyses.has(storyId))
            return;
    ...
    async analyzeAlterSystem(storyId, phase, participantId = '') {
        ...
        if (!config.enabled)
            return;
        const story = await this.getStory(storyId);
        const state = (0, alter_1.normalizeAlterSystemState)(story.state.alterSystem);
        if (!state)
            return;
        const now = new Date();
        const threshold = (0, alter_1.calculateAlterThreshold)(state.history, config, now);
        if (Math.abs(state.alterValue) < threshold || (0, alter_1.alterAnalysisCoolingDown)(state, now))
            return;
        ...
        try {
            ...
            this.reportOperation('standard', 'info', story, phase, 'Alter 累积触发 数值=%s 阈值=%s 方向=%s', signedNumber(triggerValue), threshold.toFixed(2), triggerDirection > 0 ? '严肃' : '放松');
            ...
            if (!description)
                throw new Error('Alter analysis returned an empty description.');
            const completed = (0, alter_1.completeAlterAnalysis)(state, description, threshold, now, config);
            await this.dbSet('interlude_story', { id: story.id }, {
                state: { ...story.state, alterSystem: completed }, updatedAt: now,
            });
            this.reportOperation('standard', 'info', story, phase, '情绪偏移生成完成 方向=%s 强度=%s 描述=%s', completed.emotionalOffset.direction, completed.emotionalOffset.intensity.toFixed(2), description);
    ...
    async facts(storyId, limit = this.memoryConfig.factLimit, query = '', participantId, turnQueryEmbedding) {
        ...
        const queryEmbedding = turnQueryEmbedding?.length
            ...
            : query.trim() && this.config.model.embedding?.liveQuery
                ...
                : [];
        const visible = (fact) => participantId === undefined || !fact.participantId || fact.participantId === participantId;
        const ranked = rows
            .filter(visible)
    ...
    async appendIntent(storyId, intent, now, participantId = '') {
        ...
        if (activeConsequence && !this.memoryConfig.activeConsequencesEnabled)
            return;
        const requestedExpiresAt = activeConsequence ? consequenceExpiresAt(payload) : undefined;
        const maxLifetime = Math.max(1, this.memoryConfig.activeConsequenceMaxDays) * koishi_1.Time.day;
        ...
        if (!notBefore || (!activeConsequence && notBefore <= now) || (activeConsequence && !expiresAt))
            return;
    ...
    async activeConsequencesAndExpire(storyId, now, participantId) {
        if (!this.memoryConfig.activeConsequencesEnabled)
            return [];
    ...
    async appendBrowserIntent(storyId, draft, now, fallbackParticipantId = '') {
        ...
        if (participantId && (!allowedParticipant || !this.canHandleParticipant(allowedParticipant)))
            return;
        const notBefore = new Date(now.getTime() + koishi_1.Time.second);
    ...
    async collectWebObservation(story, draft, participantId, intentId, now, persist = true) {
        ...
        if (cached) {
            if (!persist)
                return { ...cached, id: 0, intentId, accessedAt: now, createdAt: now };
            await this.appendEntry(story.id, {
        ...
        return this.withBrowserSlot(async () => {
            ...
            try {
                page = await puppeteer.page();
                await page.setUserAgent(`Mozilla/5.0 (compatible; HDS-Interlude/${meta_1.HDS_INTERLUDE_VERSION}; +https://koishi.chat/)`);
                ...
                if (!isSafePublicWebUrl(finalUrl, config))
                    throw new Error('页面重定向到了不允许的地址。');
                ...
                if (config.logObservationPreview)
                    this.report('debug', story, 'intent-due', '网页观察节选：%s', excerpt);
                return observation;
            }
            catch (error) {
    ...
    async persistCollectedWebObservation(observation) {
        return this.saveWebObservation(observation.storyId, observation.participantId, observation.intentId, observation.mode, observation.query, observation.url, observation.title, observation.excerpt, observation.summary, observation.status, observation.accessedAt);
    }
    async findCachedWebObservation(storyId, participantId, draft, now) {
        const minutes = this.browserConfig.cacheMinutes;
        if (minutes <= 0)
            return undefined;
        const cutoff = new Date(now.getTime() - minutes * koishi_1.Time.minute);
    ...
    async scheduleNarrativeRetry(storyId, participantId, now, previousAttempts = 0) {
        ...
        if (existing.length)
            await this.dbSet('interlude_intent', { id: { $in: existing.map(intent => intent.id) } }, { status: 'cancelled', updatedAt: now });
        ...
        const notBefore = new Date(now.getTime() + delaySeconds * koishi_1.Time.second);
        await this.appendIntent(storyId, {
    ...
    scheduleDueIntentWake(storyId, notBefore) {
        ...
        if (existing && existing.dueAt <= notBefore.getTime())
            return;
        if (existing)
            existing.cancel();
        const wake = () => {
            ...
            void (async () => {
                ...
                if (this.sweepRunning || this.hasPendingNarrative(storyId)) {
                    const retryAt = Date.now() + koishi_1.Time.second;
                    const retry = this.ctx.setTimeout(wake, koishi_1.Time.second);
        ...
        this.reportStandaloneOperation('diagnostic', 'debug', '已设置到期计时器 故事=%s 触发时间=%s 等待=%dms', storyId, (0, time_1.formatLogTime)(notBefore, 'Asia/Shanghai'), delay);
    }
    async scheduleNextSplitWake(storyId) {
        ...
        if (next)
            this.scheduleDueIntentWake(storyId, next.notBefore);
    }
    ...
    async appendFollowUpCommitment(story, participantId, draft, fallbackSourceEntryId, now) {
        ...
        if (duplicate || pending.length >= 2) {
            this.reportOperation('diagnostic', 'debug', story, 'user-message', '承诺回访未重复创建 参与者=%s 原因=%s', participantId, duplicate ? '同一事项待处理' : '待处理上限');
            return;
        }
        ...
        this.reportOperation('standard', 'info', story, 'user-message', '已登记承诺回访 参与者=%s 类型=%s 到期=%s', participantId, draft.kind, (0, time_1.formatLogTime)(new Date(draft.notBefore), story.setting.timezone));
    }
    async applyFollowUpResolutions(storyId, participantId, resolutions, interaction, now) {
        if (!resolutions.length || interaction?.reply.mode !== 'immediate' || !interaction.reply.content?.trim())
            return new Set();
        ...
        for (const resolution of resolutions) {
            ...
            if (resolution.outcome === 'rescheduled') {
                ...
                if (!nextAt || nextAt <= now || nextAt.getTime() - now.getTime() > 12 * koishi_1.Time.hour)
                    continue;
    ...
    async deferUnresolvedDueFollowUps(storyId, participantId, contextIntents, resolvedIds, interaction, now) {
        ...
        for (const intent of due) {
            ...
            const retryAt = new Date(now.getTime() + 20 * koishi_1.Time.minute);
            await this.dbSet('interlude_intent', { id: intent.id }, {
                notBefore: retryAt,
            ...
            this.scheduleDueIntentWake(storyId, retryAt);
            this.reportOperation('standard', 'warn', await this.getStory(storyId), 'intent-due', '承诺回访尚未给出可见结果，已保留重查 参与者=%s', participantId);
        }
    }
    async appendProactiveCheck(story, candidate, notBefore, reason, now) {
        const expiresAt = toDate(candidate.expiresAt);
        if (!expiresAt || expiresAt <= now || notBefore >= expiresAt)
            return;
        const fingerprint = (0, agency_1.proactiveCandidateFingerprint)(candidate);
        const pending = await this.dbGet('interlude_intent', {
            storyId: story.id,
        ...
        if (pending.some(intent => intent.payload?.fingerprint === fingerprint)) {
            this.reportOperation('diagnostic', 'debug', story, 'advance', 'Agency 主动联系候选去重 参与者=%s 指纹=%s', candidate.participantId, fingerprint);
            return;
        }
        await this.appendIntent(story.id, {
            type: 'proactive-check',
        ...
        this.reportOperation('standard', 'info', story, 'advance', 'Agency 已安排主动联系重查 参与者=%s 时间=%s 原因=%s', candidate.participantId, (0, time_1.formatLogTime)(notBefore, story.setting.timezone), reason);
    }
    async cancelPendingOutgoingMessages(storyId, participantId, now, cancelPlanned = true) {
        ...
        try {
            ...
            const matching = intents.filter(intent => intent.participantId === participantId && (intent.type === 'split-message'
    ...
    async resolveTTSCloneSample(source) {
        ...
        if (!base64)
            throw new Error('克隆样本为空:voice 需为 http(s) 音频链接或 data URI');
        ttsCloneSampleCache.set(source, base64);
        return base64;
    }
    async synthesizeTTSAudio(cfg, text) {
        const payload = { api_key: cfg.apiKey, text, model: cfg.model };
    ...
    async sendTTSVoice(story, transport, content, userInitiated, literalQuote) {
        const cfg = this.resolveTTSConfig();
        if (!cfg)
            return;
        if (cfg.mode === 'off')
            return;
        if (cfg.mode === 'proactive' && userInitiated)
            return;
        if (literalQuote)
            return;
        const ttsText = stripMarkupForTTSText(content);
        if (!ttsText || ttsText.length > cfg.maxCharacters)
            return;
        try {
            const { buffer, extension } = await this.synthesizeTTSAudio(cfg, ttsText);
            const tempFile = (0, node_path_1.join)((0, node_os_1.tmpdir)(), `hdsi-tts-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}${extension}`);
            await (0, promises_1.writeFile)(tempFile, buffer);
            try {
                const element = (0, koishi_1.h)('audio', { src: (0, node_url_1.pathToFileURL)(tempFile).href });
                if (transport.session)
                    await transport.session.send(element);
                else if (transport.bot)
                    await transport.bot.sendMessage(transport.channelId, element);
    ...
    async sendOutgoingMessages(story, messages, current, session, shouldCancel, recordFailures = true) {
        ...
        for (const message of messages) {
            ...
            if (!target) {
                ...
                if (recordFailures)
                    await this.recordOutgoingDeliveryFailure(story, message.participantId, message, 'participant-not-found');
                continue;
            }
            if (!this.canHandleParticipant(target)) {
                this.report('warn', story, 'intent-due', '消息被当前账号白名单拦截 参与者=%s', target.id);
                if (recordFailures)
                    await this.recordOutgoingDeliveryFailure(story, target.id, message, 'participant-not-allowed');
                continue;
            }
            ...
            try {
                ...
                if (literalQuoteOnly && !literalQuoteMessageId) {
                    ...
                    if (recordFailures)
                        await this.recordOutgoingDeliveryFailure(story, target.id, message, 'literal-quote-target-not-found');
                    continue;
                }
                if (literalQuoteMessageId)
                    message.quoteMessageId = literalQuoteMessageId;
                const outgoingContent = literalQuoteMessageId
                    ? [(0, koishi_1.h)('quote', { id: literalQuoteMessageId }), '\u200b']
                    : message.content;
                if (this.config.logging?.logMessageContent) {
                ...
                if (!bot) {
                    ...
                    if (recordFailures)
                        await this.recordOutgoingDeliveryFailure(story, target.id, message, 'bot-not-found');
                    continue;
                }
                ...
                if (recordFailures)
                    await this.recordOutgoingDeliveryFailure(story, target.id, message, `transport-error: ${String(error)}`);
            }
        }
        return delivered;
    }
    ...
    async confirmOutgoingDeliveries(story, delivered) {
        for (const message of delivered) {
            await this.serial(story.id, async () => {
                ...
                await this.recordCharacterMessage(participant, now);
                if (message.automaticDelivery)
                    await this.recordAutomaticDelivery(story.id, participant.id, message.automaticDelivery, now);
                let delay = 0;
    ...
    splitOutgoingMessage(content) {
        if (this.config.runtime.splitReplyMessages === false)
            return [content];
        const separator = this.config.runtime.messageSeparator?.trim() || '<sep/>';
        if (!separator || !content.includes(separator))
            return [content];
    ...
    typingDelayMilliseconds(nextSegment) {
        ...
        return Math.max(250, Math.min(maximumSeconds * koishi_1.Time.second, Math.round(nominal * factor * koishi_1.Time.second)));
    }
    findBotForParticipant(participant) {
        return this.ctx.bots.find(bot => String(bot.selfId) === String(participant.selfId)
    ...
    get autoAdvanceConfig() {
        if (this.cachedAutoAdvanceConfig)
    ...
    isAutomaticAdvanceDue(story, now) {
        ...
        return now.getTime() - story.cursorAt.getTime() >= config.intervalMinutes * koishi_1.Time.minute;
    }
    async pauseAutomaticAdvanceAfterUserMessage(storyId, now) {
        ...
        const story = await this.getStory(storyId);
        const fallbackNext = await this.schedulePreplanAnchoredTime(story, now, new Date(now.getTime() + automaticIntervalMinutes(story, now, this.autoAdvanceConfig) * koishi_1.Time.minute));
        const automation = {
            ...(story.state.automation ?? {}),
    ...
    async scheduleConversationFollowUpsAfterTurn(storyId, now, rawInteraction, participantId = '') {
        ...
        const ordinaryNext = followUps.at(-1) ?? new Date(anchor.getTime() + automaticIntervalMinutes(story, anchor, config) * koishi_1.Time.minute);
        ...
        await this.dbSet('interlude_story', { id: story.id }, { state: { ...story.state, automation }, updatedAt: now });
        this.reportOperation('standard', 'info', story, 'conversation-follow-up', '已更新对话后续计划 短期补写=%s 常规推进=%s', followUps.length ? followUps.map(value => (0, time_1.formatLogTime)(value, story.setting.timezone)).join('、') : '无', (0, time_1.formatLogTime)(normalNext, story.setting.timezone));
    }
    async scheduleNextAutomaticAdvance(storyId, now) {
        ...
        const ordinaryNext = new Date(now.getTime() + intervalMinutes * koishi_1.Time.minute);
        ...
        await this.dbSet('interlude_story', { id: story.id }, { state: { ...story.state, automation }, updatedAt: now });
        this.reportOperation('standard', 'info', story, 'advance', '已设置下次自动推进 时间=%s 间隔=%d分钟%s', (0, time_1.formatLogTime)(nextAdvanceAt, story.setting.timezone), Math.max(1, Math.round((nextAdvanceAt.getTime() - now.getTime()) / koishi_1.Time.minute)), nextAdvanceAt < ordinaryNext ? '（Schedule Preplan 锚点）' : '');
    }
    async schedulePreplanAnchoredTime(story, now, ordinaryNext) {
        if (!this.schedulePreplanConfig.enabled || !this.schedulePreplanConfig.anchorAutoAdvance)
            return ordinaryNext;
        const schedule = await this.getSchedulePreplan(story.id);
        const transition = (0, schedule_preplan_1.nextSchedulePreplanTransition)(schedule, now, story.setting.timezone, 12);
        return transition && transition > now && transition < ordinaryNext ? transition : ordinaryNext;
    }
    get sharedStoryConfig() {
        if (this.cachedSharedStoryConfig)
    ...
    mainModelLabel() {
        const providers = (0, narrator_1.configuredProviders)(this.config.model);
        const assigned = providers.find(item => item.enabled && item.endpoint && item.model && item.useForMain === true);
        const modelId = (0, narrator_1.effectiveMainModelId)(this.config.model);
    ...
    initialStorySetting(name) {
        const setting = (0, types_1.emptyStorySetting)();
    ...
    async resetParticipantCanon(storyId, now) {
        ...
        for (const participant of participants) {
            ...
            await this.dbSet('interlude_participant', { id: participant.id }, {
                ...
                profile: account?.profile?.trim() || preset?.profile?.trim() || this.config.storyDefaults.userProfile,
                relationship: account?.relationship?.trim() || preset?.relationship?.trim() || this.config.storyDefaults.relationship,
                state: (0, types_1.emptyParticipantState)(),
                updatedAt: now,
            });
        }
    }
    ...
    async migrateLegacyStory(legacy, session) {
        ...
        for (const table of tables)
            await this.dbSet(table, { storyId: legacy.id }, { storyId: story.id });
    ...
    async ensureContinuity(story, now) {
        ...
        if (!scene) {
            ...
            if (arc)
                await this.dbSet('interlude_arc', { id: arc.id }, { sceneCount: arc.sceneCount + 1, updatedAt: now });
        }
    ...
    scheduleCompaction(storyId) {
        if ((!this.memoryConfig.enabled && !this.schedulePreplanConfig.enabled) || this.scheduledCompactions.has(storyId))
            return;
        ...
        const run = () => {
            ...
            void (async () => {
                ...
                if (!prepared || (!prepared.review?.needsModel && (!prepared.context || prepared.context.phase === 'skip')))
                    return;
                ...
                let scheduleProposal = undefined;
                ...
                await this.serial(storyId, async () => {
                    ...
                    if (prepared.review?.needsModel) {
                        ...
                        if (persisted)
                            this.schedulePreplanBackoff.delete(storyId);
                        else
                            this.schedulePreplanBackoff.set(storyId, Date.now() + SCHEDULE_PREPLAN_RETRY_BACKOFF);
                    }
                    if (context)
    ...
    async compactStories() {
        if ((!this.memoryConfig.enabled && !this.schedulePreplanConfig.enabled) || this.compactionSweepRunning)
            return;
    ...
    async saveSchedulePreplan(record) {
        ...
        if (existing)
            await this.dbSet('interlude_schedule_preplan', { storyId: record.storyId }, record);
        else
            await this.dbCreate('interlude_schedule_preplan', record);
    }
    async prepareSchedulePreplanReview(story, now) {
        ...
        if (backoffUntil && now.getTime() < backoffUntil)
            return undefined;
        const current = await this.getSchedulePreplan(story.id);
        if (!(0, schedule_preplan_1.schedulePreplanReviewDue)(current, now, story.setting.timezone, config))
            return undefined;
        const localDate = (0, time_1.calendarDayKey)(now, story.setting.timezone);
        ...
        if (!current && !evidenceEntries.length) {
            const empty = (0, schedule_preplan_1.applySchedulePreplanProposal)(undefined, { outcome: 'replace', reason: 'No concrete recurring schedule evidence yet.', regimes: [], exceptions: [] }, [], localDate, story.setting.timezone, config, now, config.variationLevel);
            if (empty) {
        ...
        const needsModel = (0, schedule_preplan_1.schedulePreplanNeedsModel)(current, evidenceEntries, localDate, story.setting.timezone, config);
        if (!needsModel && current) {
            await this.saveSchedulePreplan((0, schedule_preplan_1.refreshSchedulePreplan)(current, localDate, story.setting.timezone, config, now));
    ...
    async requestSchedulePreplan(story, request) {
        if (!this.compactor.planSchedulePreplan)
            return undefined;
    ...
    async persistSchedulePreplanReview(story, review, proposal, now) {
        const next = (0, schedule_preplan_1.applySchedulePreplanProposal)(review.current, proposal, review.evidenceEntries, review.localDate, story.setting.timezone, this.schedulePreplanConfig, now, this.schedulePreplanConfig.variationLevel);
        if (!next) {
            ...
            if (!review.current) {
                const empty = (0, schedule_preplan_1.applySchedulePreplanProposal)(undefined, { outcome: 'replace', reason: 'Schedule review returned no valid structure; waiting for new concrete evidence.', regimes: [], exceptions: [] }, review.evidenceEntries, review.localDate, story.setting.timezone, this.schedulePreplanConfig, now, this.schedulePreplanConfig.variationLevel);
                if (empty) {
                    empty.storyId = story.id;
                    await this.saveSchedulePreplan(empty);
    ...
    async compactUnlocked(story, now, force) {
        ...
        if (review?.needsModel && review.request) {
            ...
            if (persisted)
                ...
                this.schedulePreplanBackoff.set(story.id, Date.now() + SCHEDULE_PREPLAN_RETRY_BACKOFF);
        }
        if (!context || context.phase === 'skip')
            return context?.overlayCompacted ?? false;
        const startedAt = Date.now();
    ...
    async compactOverlayUnlocked(story, now) {
        ...
        if (!config.overlayCompressionEnabled)
            return false;
        try {
            const recentCutoff = new Date(now.getTime() - (config.overlayRecentDays ?? 2) * koishi_1.Time.day);
            const monthlyCutoff = new Date(now.getTime() - (config.overlayMonthlyAfterDays ?? 10) * koishi_1.Time.day);
            ...
            for (const group of groupOverlayPatches(weekly, config.overlayWeeklyWindowDays ?? 5)) {
                ...
                for (const patch of group.patches)
                    await this.dbSet('interlude_state_patch', { id: patch.id }, { status: 'compacted' });
                changed = true;
            }
    ...
    async overlaySnapshotsForPrompt(storyId, participantId, background = false) {
        if (!this.memoryConfig.overlayCompressionEnabled)
            return [];
    ...
    async rebuildLiveOverlayState(story, now) {
        ...
        if (hasGlobalHistory('character')) {
            ...
            for (const patch of applied.filter(item => !item.participantId && item.target === 'character')) {
                if (patch.path.includes('trait'))
                    overlay.characterTraits.push(clip(patch.proposedValue, 500));
                else
                    overlay.characterProfile = mergeNote(overlay.characterProfile, patch.proposedValue);
            }
        ...
        if (hasGlobalHistory('world')) {
            ...
            for (const patch of applied.filter(item => !item.participantId && item.target === 'world'))
                overlay.world = mergeNote(overlay.world, patch.proposedValue);
        }
        if (hasGlobalHistory('relationship')) {
            overlay.relationship = undefined;
            for (const patch of applied.filter(item => !item.participantId && item.target === 'relationship'))
                overlay.relationship = mergeNote(overlay.relationship, patch.proposedValue);
        }
    ...
    async persistCompaction(story, scene, decision, entries, now, visibleFactIds = new Set()) {
        ...
        if (decision.workingDetails?.length) {
            ...
            for (const item of state.workingDetails ?? [])
                merged.set(item.label, item);
            for (const draft of decision.workingDetails) {
                if (!hasCompactionEvidence(draft.sourceEntryIds ?? [], entries))
                    continue;
        ...
        for (const fact of decision.facts ?? []) {
            if (!hasCompactionEvidence(fact.sourceEntryIds, entries))
                continue;
        ...
        for (const patch of decision.statePatches ?? []) {
            if (!hasCompactionEvidence(patch.sourceEntryIds, entries))
                continue;
            await this.persistStatePatch(story, patch, entries, now);
        }
        if (resolvedFacts)
            await this.markContinuityDirty(story.id, now);
    }
    async persistFact(storyId, draft, entries, now) {
        ...
        if (existing.length >= this.memoryConfig.maxFactsPerStory) {
            ...
            if (oldest)
                await this.dbSet('interlude_fact', { id: oldest.id }, { status: 'superseded', updatedAt: now });
        }
        await this.dbCreate('interlude_fact', {
    ...
    scheduleFactEmbeddingBackfill(storyId) {
        ...
        if (!embedding?.enabled || !embedding.model?.trim() || batchSize <= 0)
            return;
        if (this.factBackfills.has(storyId))
            return;
    ...
    async backfillFactEmbeddings(storyId, batchSize) {
        ...
        for (const fact of missing) {
            ...
            if (embedding.length)
                await this.dbSet('interlude_fact', { id: fact.id }, { embedding, updatedAt: new Date() });
        }
    }
    async persistStatePatch(story, draft, entries, now) {
        ...
        if (!path || !proposedValue || !sourceEntryIds.length)
            return;
        ...
        if (matching.some(candidate => candidate.status === 'applied' || candidate.status === 'compacted'))
            return;
        ...
        if (!this.memoryConfig.autoApplyStatePatches || (impact === 'major' && !this.memoryConfig.allowMajorStateChanges))
            return;
        ...
        if (!stableEvidence) {
            this.reportOperation('diagnostic', 'debug', story, 'advance', 'Overlay 候选继续累计 目标=%s/%s 回合=%d/%d 日期=%d/%d', draft.target, path, evidence.turns, minimumTurns, evidence.days, minimumDays);
            return;
        }
        ...
        if (recentApplied && now.getTime() - recentApplied.getTime() < cooldownHours * koishi_1.Time.hour) {
            this.reportOperation('diagnostic', 'debug', story, 'advance', 'Overlay 冷却中，候选保留 目标=%s/%s 冷却=%d小时', draft.target, path, cooldownHours);
            return;
        }
        const overlay = { ...(story.state.settingOverlay ?? {}) };
        if (draft.target === 'character') {
            if (draft.path.includes('trait'))
                overlay.characterTraits = Array.from(new Set([...(overlay.characterTraits ?? []), clip(draft.proposedValue, 500)])).slice(-30);
            else
                overlay.characterProfile = mergeNote(overlay.characterProfile, draft.proposedValue);
        }
        ...
        else if (draft.target === 'relationship')
            overlay.relationship = mergeNote(overlay.relationship, draft.proposedValue);
        else
            overlay.world = mergeNote(overlay.world, draft.proposedValue);
        if (draft.target !== 'relationship' || !participantId) {
            const state = { ...story.state, settingOverlay: overlay };
            await this.dbSet('interlude_story', { id: story.id }, { state, updatedAt: now });
        }
        if (proposal?.id)
            await this.dbSet('interlude_state_patch', { id: proposal.id }, { status: 'applied', appliedAt: now });
    }
    ...
    reportOperation(verbosity, level, story, phase, message, ...args) {
        if (!this.allowsVerbosity(verbosity))
            return;
        this.writeReport(level, story, phase, message, args);
    }
    writeReport(level, story, phase, message, args) {
        if (this.blindModeConfig.enabled) {
            if (level === 'error' || level === 'warn')
                this.blindModeHealthIssue = true;
            return;
        }
        ...
        if (rank[logging.level] < rank[level])
            return;
        const rendered = (0, logging_1.renderLogMessage)(message, args);
        const storyDetail = (logging.verbosity ?? 'standard') === 'diagnostic' ? ` 故事=${story.id}` : '';
        const output = logging.format === 'layered'
            ? (0, logging_1.formatLayeredLog)({
            ...
            : logging.format === 'compact'
                ? `[${(0, logging_1.phaseLabel)(phase)}] ${story.setting.character.name} ${rendered}${storyDetail}`
                : `[${(0, logging_1.phaseLabel)(phase)}] ${story.setting.character.name}\n事件：${rendered}${storyDetail}`;
    ...
    reportTokenUsage(record) {
        const line = (0, narrator_1.formatTokenUsageLine)(record);
        if (!line)
            return;
    ...
    writeStandalone(level, message, args) {
        ...
        const output = logging.format === 'layered'
            ...
            : `[系统] ${(0, logging_1.renderLogMessage)(message, args)}`;
        this.emitLog(level, output);
    }
    async resolveCompactionFacts(storyId, value, allowedIds, now) {
        ...
        if (!ids.length)
            return false;
        const facts = await this.dbGet('interlude_fact', { storyId, id: { $in: ids }, status: 'active' });
    ...
    emitLog(level, output) {
        if (level === 'error')
            this.serviceLogger.error(output);
        else if (level === 'warn')
            this.serviceLogger.warn(output);
        else if (level === 'info')
            this.serviceLogger.info(output);
        else
            this.serviceLogger.debug(output);
    }
    ...
    async getStory(id) {
        ...
        if (!story)
            throw new Error(`Interlude story not found: ${id}`);
        return story;
    }
    serial(id, task) {
    ...
    async repairCanonicalOneBotStoryTransport(story, session) {
        if (!isOneBotPlatform(session.platform) || !session.selfId)
            return story;
        ...
        if (hasLiveStoryBot || (story.platform === session.platform && String(story.selfId) === String(session.selfId)))
            return story;
        const now = new Date();
...
exports.InterludeService = InterludeService;
function storyIdForCharacter(platform, selfId) { return `character:${platform}:${selfId}`; }
function legacyStoryIdFor(platform, selfId, userId) { return `${platform}:${selfId}:${userId}`; }
function participantIdFor(platform, selfId, userId) { return `${platform}:${selfId}:${userId}`; }
...
function extractSessionImageSources(session) {
    ...
    const add = (value, kind = 'url') => {
        ...
        if (/^https?:\/\//i.test(source))
            sources.push(kind === 'adapter-url' ? `onebot-url:${source}` : source);
        else if (/^data:image\//i.test(source))
            sources.push(source);
        else if (kind === 'file')
            sources.push(`onebot-file:${source}`);
    };
    const visit = (element) => {
        if (!element)
            return;
        ...
        if (type === 'img' || type === 'image') {
            ...
            if (src)
                ...
                add(element.attrs?.file ?? element.data?.file, 'file');
        }
    ...
    try {
        for (const element of koishi_1.h.parse(raw))
            visit(element);
    }
    catch { }
    if (!sources.length) {
    ...
    for (let match = cqPattern.exec(raw); match; match = cqPattern.exec(raw)) {
        ...
        if (!fields.url && !fields.cache_url)
...
function mergeUserMessageWithVoiceTranscripts(text, transcripts, detected = 0) {
    const parts = [clip(text, 8_000)];
    for (const [index, transcript] of transcripts.slice(0, 1).entries()) {
        const value = clip(transcript, 4_000);
        if (value)
            parts.push(`[用户语音转写 ${index + 1}]\n${value}`);
    }
    if (detected > 0 && transcripts.length === 0)
        parts.push('[用户发送了一段语音；未能转写其内容。]');
...
const CHAT_REACTION_NAMES = ['like', 'smile', 'laugh', 'heart', 'surprised', 'sad', 'angry'];
const QQ_REACTION_IDS = {
    like: '76', smile: '14', laugh: '182', heart: '66', surprised: '0', sad: '5', angry: '106',
};
const NATIVE_FACE_SEMANTICS = ['smile', 'laugh', 'sweat', 'awkward', 'heart', 'surprised', 'sad', 'angry'];
const QQ_NATIVE_FACE_IDS = {
...
async function listStickerFiles(root) {
    ...
    const visit = async (directory, depth) => {
        ...
        for (const entry of entries) {
            const full = (0, node_path_1.resolve)(directory, entry.name);
...
function extractUserReportedTimes(content, now, timezone) {
    ...
    const currentMinutes = (0, time_1.localClockMinutes)(now, timezone);
    const date = (0, time_1.calendarDayKey)(now, timezone);
    const facts = [];
    const seen = new Set();
    ...
    for (const match of text.matchAll(/(?:今天|今晚|下午|晚上|早上|上午)?\s*(\d{1,2})\s*(?:[:：.]|点)\s*(\d{2}|半)?/g)) {
        ...
        if ((prefix.includes('下午') || prefix.includes('晚上')) && hour < 12)
            ...
            if (Math.abs(evening - currentMinutes) < Math.abs(morning - currentMinutes))
                hour += 12;
        }
...
function normalizeQuotedMessageContent(value) {
    const raw = (0, qq_face_1.normalizeQQNativeFaceSegments)(value);
    const content = raw
...
function normalizeTimelinePlan(value) {
    ...
    const beats = value.beats
        ...
        .filter((item) => Number.isFinite(item.at) && !!item.kind && !!item.summary)
...
function visibleReplyMode(decision, phase, groupContext) {
    if (phase === 'advance') {
        if (decision.crossConversationActions?.some(action => action.mode === 'immediate'))
            return '主动联系';
        if (decision.crossConversationActions?.some(action => action.mode === 'delayed'))
            return '计划联系';
        return '无可见投递';
    }
    if (phase === 'conversation-follow-up' || phase === 'intent-due') {
        if (hasStructuredInteraction(decision.interaction))
    ...
    if (!groupContext)
        return hasStructuredInteraction(decision.interaction) ? decision.interaction.reply.mode : '未提供或无效';
    if (hasStructuredGroupReplyField(decision.groupReply))
        return `group:${decision.groupReply.mode}`;
    if (hasStructuredInteraction(decision.interaction))
        return `group-fallback:${decision.interaction.reply.mode}`;
    return '未提供或无效';
}
...
function hasStructuredGroupReplyField(value) {
    if (!isRecord(value) || (value.mode !== 'none' && value.mode !== 'immediate'))
        return false;
    return value.mode === 'none' || typeof value.content === 'string' && !!value.content.trim();
}
function hasStructuredInteraction(value) {
    if (!isRecord(value) || typeof value.seen !== 'boolean' || !isRecord(value.reply))
        return false;
    ...
    if (mode === 'none')
        return true;
    if (typeof value.reply.content !== 'string' || !value.reply.content.trim())
        return false;
...
function samePlatformFamily(left, right) {
    if (isOneBotPlatform(left) && isOneBotPlatform(right))
        return true;
...
exports.resolveBlackBoxConfig = resolveBlindModeConfig;
...
function followUpExpiresAt(value, now) {
    ...
    const maximum = new Date(now.getTime() + 24 * koishi_1.Time.hour);
...
function normalizeFollowUpCommitment(value, now) {
    ...
    if (!kind || !summary || !notBefore || notBefore.getTime() - now.getTime() < 5 * koishi_1.Time.minute || notBefore.getTime() - now.getTime() > 12 * koishi_1.Time.hour)
        return undefined;
...
function inferredFollowUpCommitment(content, now) {
    return {
        ...
        notBefore: new Date(now.getTime() + 20 * koishi_1.Time.minute).toISOString(),
    };
}
...
function mergeDeliverySummary(left, right) {
    if (!left || left === right || left.includes(right))
        return left || right;
...
function normalizeAutomaticDeliverySummaries(value) {
    ...
    for (const item of value) {
        ...
        if (!participantId || !summary || !deliveredAt || seen.has(key))
            continue;
        seen.add(key);
...
function normalizeScenePresenceState(value) {
    ...
    for (const item of value) {
        ...
        const updatedAt = typeof item.updatedAt === 'string' && !Number.isNaN(new Date(item.updatedAt).getTime())
            ? item.updatedAt
            : '';
        if (!name || !status || !basis || !sourceEntryIds.length || !updatedAt)
            continue;
...
function normalizeScenePresenceDrafts(value, entries, now = new Date()) {
    if (!Array.isArray(value))
        return [];
    ...
    for (const item of value) {
        ...
        if (!name || !status || !basis || !evidence.length || !hasExplicitPresenceEvidence(status, evidence))
            continue;
...
function hasExplicitPresenceEvidence(status, entries) {
    ...
    if (status === 'off-scene')
        return /告别|道别|分别|先走|离开|离去|回家|回去了|独自|分开|告辞/.test(text);
    if (status === 'expected')
        return /约好|约在|等会|稍后|会来|准备来|约见/.test(text);
...
function normalizeDecision(raw, from, now, permitMessages, runtime, shared, currentParticipantId, permittedParticipantIds, phase = 'advance', memory, refreshContinuity = false) {
    ...
    const browserIntents = Array.isArray(raw?.browserIntents)
        ? raw.browserIntents.map(normalizeBrowserIntentDraftLoose).filter((intent) => !!intent).slice(0, 1)
        : [];
    ...
    const alter = (0, alter_1.normalizeAlterValue)(raw?.alter);
...
function normalizeContinuitySnapshot(value) {
    if (!isRecord(value))
        return undefined;
    const text = (item, limit) => typeof item === 'string' ? clip(item, limit).trim() : '';
    const list = (item, limit) => Array.isArray(item)
...
function normalizeBrowserIntentDraft(draft, config) {
    ...
    if (!normalized)
        return undefined;
    if (normalized.mode === 'search' && !config.allowSearch)
        return undefined;
    if (normalized.mode === 'visit' && !config.allowVisit)
        return undefined;
    return normalized;
}
...
function isSafePublicWebUrl(value, config) {
    try {
        ...
        if (blocked.some(domain => domainMatches(host, domain)))
            return false;
...
function normalizeInteraction(value, now, runtime) {
    ...
    if (!value.seen)
        return { seen: false, reply: { mode: 'none' } };
    if (mode === 'none')
        return { seen: true, reply: { mode: 'none' } };
    if (!content)
        return { seen: true, reply: { mode: 'none' } };
    if (mode === 'immediate')
        return { seen: true, reply: { mode, content } };
    const delay = sendAt?.getTime() - now.getTime();
    if (!sendAt || delay < runtime.minimumDelayedReplySeconds * 1_000 || delay > runtime.maximumDelayedReplyMinutes * koishi_1.Time.minute)
        return { seen: true, reply: { mode: 'none' } };
...
function validIntent(value, from, now, memory) {
    ...
    if (!notBefore)
        return false;
    if (!isActiveConsequenceDraft(value))
    ...
    const maximumLifetime = Math.max(1, memory?.activeConsequenceMaxDays ?? 7) * koishi_1.Time.day;
...
function normalizeIntentUpdates(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .filter((item) => isRecord(item) && Number.isInteger(item.id) && Number(item.id) > 0 && (item.status === 'completed' || item.status === 'cancelled'))
        .map(item => ({
        id: Number(item.id), status: item.status,
...
function normalizeConversationAction(value, runtime, permittedParticipantIds, currentParticipantId, now = new Date(), proactive = false) {
    if (!isRecord(value) || typeof value.participantId !== 'string' || !value.participantId || value.participantId === currentParticipantId)
        return undefined;
    if (!permittedParticipantIds.has(value.participantId) || (value.mode !== 'immediate' && value.mode !== 'delayed'))
        return undefined;
    ...
    const willingness = typeof value.willingness === 'number' && Number.isFinite(value.willingness)
        ...
        : undefined;
    if (proactive && (willingness === undefined || willingness < (runtime.proactiveWillingnessThreshold ?? 0.65)))
        return undefined;
    const reason = typeof value.reason === 'string' ? clip(value.reason, 300) : undefined;
    if (value.mode === 'immediate')
        return { participantId: value.participantId, mode: value.mode, content, ...(willingness === undefined ? {} : { willingness }), ...(reason ? { reason } : {}) };
...
function permittedOrGlobal(value, fallback, permittedParticipantIds) {
    ...
    if (candidate && permittedParticipantIds.has(candidate))
        return candidate;
    return fallback && permittedParticipantIds.has(fallback) ? fallback : '';
}
function pickParticipantStatePatch(value) {
    const patch = {};
    if (Array.isArray(value.openThreads) && value.openThreads.every(item => typeof item === 'string'))
        patch.openThreads = value.openThreads.map(item => clip(item, 500)).slice(0, 50);
    if (Array.isArray(value.relationshipNotes) && value.relationshipNotes.every(item => typeof item === 'string'))
        patch.relationshipNotes = value.relationshipNotes.map(item => clip(item, 500)).slice(0, 50);
    return patch;
}
...
function normalizeStoryState(value) {
    ...
    return {
        ...
        alterSystem: (0, alter_1.normalizeAlterSystemState)(record.alterSystem),
        agencyWindow: (0, agency_1.normalizeAgencyWindowState)(record.agencyWindow),
...
function clip(value, length) { return typeof value === 'string' ? value.trim().slice(0, length) : ''; }
function clampNumber(value, fallback, min, max) {
    if (typeof value !== 'number' || Number.isNaN(value))
        return fallback;
    return Math.max(min, Math.min(max, value));
}
function normalizeFact(value) { return value.trim().toLocaleLowerCase().replace(/\s+/g, ' '); }
function limitEntriesByCharacters(entries, limit) {
    ...
    for (let index = entries.length - 1; index >= 0; index--) {
        ...
        if (selected.length && used + entry.content.length > limit)
            break;
...
function factScore(fact, config, queryEmbedding = []) {
    const ageDays = Math.max(0, (Date.now() - fact.lastSeenAt.getTime()) / (24 * koishi_1.Time.hour));
...
exports.SEMANTIC_STICKER_LIMIT = 12;
...
function rankStickerCatalog(assets, queryEmbedding, limit) {
    if (!queryEmbedding.length || assets.length <= limit)
        return assets;
    return assets
...
function automaticIntervalMinutes(story, now, config) {
    ...
    if (restWindow)
        return randomInteger(restWindow.minIntervalMinutes, restWindow.maxIntervalMinutes);
...
function scheduleConversationFollowUps(anchor, config) {
    ...
    return config.followUpMinutes.map(minutes => {
        ...
        const at = Math.max(previous + 1_000, anchor.getTime() + Math.max(1, minutes + jitter) * koishi_1.Time.minute);
        previous = at;
        return new Date(at);
    });
}
function activeRestWindow(windows, timezone, now) {
    const localMinutes = (0, time_1.localClockMinutes)(now, timezone);
...
function mergeNote(existing, next) {
    ...
    if (!existing)
        return value;
    if (normalizeFact(existing).includes(normalizeFact(value)))
        return existing;
    return `${existing}\n${value}`.slice(-6_000);
}
...
function statePatchEvidence(entries, timezone) {
    ...
    const days = new Set(narrative.map(entry => (0, time_1.calendarDayKey)(entry.occurredAt, timezone))).size;
    return { turns, days };
}
function startOfUtcWindow(value, windowDays) {
    const size = Math.max(1, Math.floor(windowDays));
    const epochDay = Math.floor(value.getTime() / koishi_1.Time.day);
    return new Date(Math.floor(epochDay / size) * size * koishi_1.Time.day);
}
function groupOverlayPatches(patches, windowDays = 5) {
    ...
    for (const patch of patches) {
        ...
        const group = groups.get(key) ?? { participantId: patch.participantId, target: patch.target, from, to: new Date(from.getTime() + windowDays * koishi_1.Time.day), patches: [] };
...
function groupOverlaySnapshots(snapshots, windowDays = 10) {
    ...
    for (const snapshot of snapshots) {
        ...
        const group = groups.get(key) ?? { participantId: snapshot.participantId, target: snapshot.target, from, to: new Date(from.getTime() + windowDays * koishi_1.Time.day), snapshots: [] };
