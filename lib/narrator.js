...
exports.OpenAICompatibleNarrator = exports.OpenAICompatibleEmbedder = exports.SilentEmbedder = exports.SilentCompactor = exports.SilentNarrator = exports.ZHIPU_FIRST_VISIBLE_TOKEN_TIMEOUT = exports.ZHIPU_OFFICIAL_CHAT_ENDPOINT = exports.storyLocalTimeContext = void 0;
exports.createNarrator = createNarrator;
exports.createStickerDescriber = createStickerDescriber;
exports.createVisionDescriber = createVisionDescriber;
exports.effectiveMainModelId = effectiveMainModelId;
exports.configuredProviders = configuredProviders;
exports.usesRemoteProviders = usesRemoteProviders;
exports.createCompactor = createCompactor;
exports.createEmbedder = createEmbedder;
exports.extractEarlyNarrativeReply = extractEarlyNarrativeReply;
exports.parseTokenUsage = parseTokenUsage;
exports.aggregateTokenUsages = aggregateTokenUsages;
exports.computeTokenCost = computeTokenCost;
exports.formatTokenUsageLine = formatTokenUsageLine;
exports.systemPrompt = systemPrompt;
exports.storyStateForPrompt = storyStateForPrompt;
exports.recentScriptOwnership = recentScriptOwnership;
exports.toPromptPayload = toPromptPayload;
exports.compactScriptTag = compactScriptTag;
exports.promptVisibleMessageContent = promptVisibleMessageContent;
const time_1 = require("./time");
var time_2 = require("./time");
Object.defineProperty(exports, "storyLocalTimeContext", { enumerable: true, get: function () { return time_2.storyLocalTimeContext; } });
exports.ZHIPU_OFFICIAL_CHAT_ENDPOINT = 'https://open.bigmodel.cn/api/paas/v4/chat/completions';
exports.ZHIPU_FIRST_VISIBLE_TOKEN_TIMEOUT = 45_000;
...
exports.SilentNarrator = SilentNarrator;
class SilentCompactor {
    async compact() { return {}; }
    async compactOverlay() { return { summary: '' }; }
    async planSchedulePreplan() { return undefined; }
    async planTimeline() { return undefined; }
}
exports.SilentCompactor = SilentCompactor;
/** A no-op embedder lets memory retrieval fall back to rule-based ranking. */
class SilentEmbedder {
    async embed() { return []; }
}
exports.SilentEmbedder = SilentEmbedder;
/**
...
class OpenAICompatibleEmbedder {
    ...
    async embed(input) {
        ...
        if (!embedding?.enabled || (!assigned && !embedding.modelId?.trim() && !embedding.model?.trim()))
            return [];
...
class OpenAICompatibleNarrator {
    ...
    async decide(request) {
        ...
        if (!providers.length)
            throw new Error('No enabled OpenAI-compatible provider is available.');
        const failures = [];
        const usages = [];
            ...
            for (const provider of providers) {
                ...
                for (let attempt = 1; attempt <= attempts; attempt++) {
                    try {
                        ...
                        if (earlyReplyCommitted)
                            throw new Error(`Narrative stream failed after an early visible reply: ${detail}`);
    ...
    async compact(request) {
        ...
        try {
            ...
            if (!text)
                throw new Error('Compaction provider returned an empty response.');
            try {
    ...
    async planTimeline(request) {
        ...
        try {
            ...
            if (!text)
                return undefined;
            return parseJsonResponse(text, 'Timeline director');
        }
        catch (error) {
    ...
    async planSchedulePreplan(request) {
        ...
        try {
            ...
            if (!text)
                return undefined;
            return parseJsonResponse(text, 'Schedule Preplan provider');
        }
        catch (error) {
    ...
    async compactOverlay(request) {
        ...
        try {
            ...
            if (!text)
                throw new Error('Overlay compaction provider returned an empty response.');
            try {
    ...
    async analyzeAlter(request, alterConfig) {
        ...
        const providers = assigned.length ? assigned : this.selectProviders(false, route.providerId);
        if (!providers.length)
            throw new Error('No enabled provider is available for Alter System analysis.');
        const failures = [];
        const usages = [];
        try {
            for (const provider of providers) {
                ...
                for (let attempt = 1; attempt <= attempts; attempt++) {
                    try {
                        ...
                        if (!text)
                            throw new Error('Alter analysis provider returned an empty response.');
                        const decision = parseJsonResponse(text, 'Alter analysis provider');
                        const description = typeof decision.description === 'string' ? decision.description.trim().slice(0, 800) : '';
                        if (!description)
                            throw new Error('Alter analysis provider returned no description.');
                        this.cooldownUntil.delete(providerKey(provider));
                        return { description };
                    }
                    catch (error) {
    ...
    async describeSticker(dataUri, mimeType, fileName, animated, responseFormat = 'json-object') {
        ...
        try {
            ...
            if (!text)
                return undefined;
            try {
                const parsed = parseJsonResponse(text, 'Sticker description provider');
    ...
    async describeImages(images, userText = '', detail = 'auto') {
        ...
        if (!providers.length || !images.length)
            return undefined;
        const usages = [];
        const failures = [];
        try {
            ...
            if (failures.length)
                this.logger?.debug('侧端识图不可用：%s', failures.join(' | '));
            return undefined;
        }
        finally {
    ...
    collectUsage(usages, task, provider, model, raw) {
        ...
        if (!hasUsageFields({ task, providerLabel: provider.label, model, ...parsed }))
            return;
        usages.push({
    ...
    emitUsage(task, usages) {
        if (!this.onUsage || !usages.length)
            return;
        const aggregated = aggregateTokenUsages(usages.map(item => ({ ...item, task })));
        if (!aggregated || !hasUsageFields(aggregated))
            return;
    ...
    async requestProvider(provider, request, overrides = {}, usages = [], task = '主叙事') {
        ...
        const onStreamText = streamingEarlyReply ? async (text) => {
            if (earlyReplyHandled)
                return;
            const reply = extractEarlyNarrativeReply(text, !!request.groupContext);
            if (reply && await request.onEarlyReply(reply))
                earlyReplyHandled = true;
        } : undefined;
        const headers = {
            'content-type': 'application/json',
            ...(provider.apiKey ? { authorization: `Bearer ${provider.apiKey}` } : {}),
        ...
        if (!text)
            throw new Error('Narrative provider returned an empty response.');
        try {
...
exports.OpenAICompatibleNarrator = OpenAICompatibleNarrator;
...
function normalizeProvider(provider) {
    ...
    return {
        ...
        timeout: provider.timeout ?? (zhipuOfficial ? exports.ZHIPU_FIRST_VISIBLE_TOKEN_TIMEOUT : 60_000),
...
function presetEndpoint(mode, dashscopeRegion) {
    if (mode === 'zhipu-official')
        return exports.ZHIPU_OFFICIAL_CHAT_ENDPOINT;
...
function createCompactor(ctx, config, silentLogs = false, onUsage) {
    if (!usesRemoteProviders(config) || config.compaction?.enabled === false)
        return new SilentCompactor();
...
async function requestZhipuStreaming(endpoint, body, headers, onText, collectUsage) {
    ...
    }, exports.ZHIPU_FIRST_VISIBLE_TOKEN_TIMEOUT);
    try {
        ...
        if (!response.body)
            throw new Error('Zhipu returned no streaming response body.');
        ...
        if (!receivedVisibleToken)
            throw new Error('Zhipu stream ended without visible content.');
        return content;
    }
    catch (error) {
        if (firstTokenTimedOut)
            throw new Error(`Zhipu first visible token timed out after ${exports.ZHIPU_FIRST_VISIBLE_TOKEN_TIMEOUT}ms.`);
        throw error;
    }
    finally {
        clearTimeout(firstTokenTimer);
...
async function requestOpenAICompatibleStreaming(endpoint, body, headers, timeout, onText, collectUsage) {
    ...
    try {
        ...
        if (!response.body)
            throw new Error('Streaming provider returned no response body.');
        const reader = response.body.getReader();
        const decoder = new TextDecoder();
        let pending = '';
        let content = '';
...
function extractEarlyNarrativeReply(raw, group) {
    ...
    if (group) {
        ...
        if (reply.mode !== 'immediate' || !content)
            return undefined;
    ...
    const content = typeof reply?.content === 'string' ? reply.content.trim() : '';
    if (typeof interaction.seen !== 'boolean' || reply?.mode !== 'immediate' || !content)
        return undefined;
...
function readJsonValueEnd(raw, start) {
    ...
    for (let index = start; index < raw.length; index++) {
        ...
        if (character === '{' || character === '[')
            ...
            if (!open || open === '{' && character !== '}' || open === '[' && character !== ']')
                return undefined;
            if (!stack.length)
                return index + 1;
        }
...
function parseTokenUsage(usage) {
    ...
    if (typeof record.prompt_cache_hit_tokens === 'number')
        cachedInputTokens = record.prompt_cache_hit_tokens;
    const result = {};
...
function formatTokenUsageLine(record) {
    ...
    if (record.outputTokens != null)
        parts.push(`输出=${record.outputTokens}`);
...
function flattenChatText(value) {
    ...
    if (Array.isArray(value))
        return value.map(item => flattenChatText(item)).join('');
    if (!value || typeof value !== 'object')
        return '';
    const record = value;
    ...
    if (typeof record.content === 'string' || Array.isArray(record.content))
        return flattenChatText(record.content);
    if (typeof record.output_text === 'string' || Array.isArray(record.output_text))
        return flattenChatText(record.output_text);
    return '';
}
...
function storyStateForPrompt(state) {
    const { alterSystem: _internalAlterSystem, agencyWindow: _internalAgencyWindow, automaticDeliverySummaries: _automaticDeliverySummaries, continuitySnapshot: _internalContinuitySnapshot, continuityDirty: _internalContinuityDirty, 
    ...
    timelineCarry: _internalTimelineCarry, ...publicState } = state;
    return publicState;
}
function recentScriptOwnership(entry) {
    if (entry.kind === 'group-message')
        return 'external-group-message';
    if (entry.kind === 'user-message' || entry.actor === 'user')
    ...
    if (entry.kind === 'script' || entry.actor === 'narrator')
        return 'protagonist-narrative';
    return 'system-event';
}
function toPromptPayload(request, options) {
    // 这是 token 预算后的连续性快照：近处使用原文，远处使用摘要和事实，而非全量历史。
    const fromLocalContext = (0, time_1.storyLocalTimeContext)(request.from, request.story.setting.timezone);
    const nowLocalContext = (0, time_1.storyLocalTimeContext)(request.now, request.story.setting.timezone);
    ...
    const payload = {
        ...
        participants: request.participants.map(participant => participantPromptPayload(participant, false, request.shareParticipantDetails || request.phase === 'advance' && request.agencyEnabled === true)),
        ...
        recentScript: compactPromptEntries(request.recentEntries, 12_000, request.recentProtectionSince).map(entry => ({
            ...
            occurredAt: entry.occurredAt.toISOString(), occurredAtLocal: (0, time_1.storyLocalTimeContext)(entry.occurredAt, request.story.setting.timezone).local,
        })),
    };
...
function buildRecentExchange(request, maxCharacters = 1_600) {
    ...
    for (let index = request.recentEntries.length - 1; index >= 0 && items.length < 3; index--) {
        ...
        if (!['user-message', 'character-message', 'character-platform-action'].includes(entry.kind))
            continue;
        if (request.phase === 'user-message' && entry.kind === 'user-message' && entry.content === request.userMessage)
            continue;
