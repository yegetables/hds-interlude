...
exports.registerTables = registerTables;
function registerTables(ctx) {
    ...
    if (existingTables.interlude_story) {
        if (!existingTables.interlude_web_observation)
            registerWebObservationTable(ctx);
        if (!existingTables.interlude_overlay_snapshot)
            registerOverlaySnapshotTable(ctx);
        if (!existingTables.interlude_sticker)
            registerStickerTable(ctx);
        if (!existingTables.interlude_schedule_preplan)
...
function registerWebObservationTable(ctx) {
    if (ctx.model.tables?.interlude_web_observation)
        return;
...
function registerOverlaySnapshotTable(ctx) {
    if (ctx.model.tables?.interlude_overlay_snapshot)
        return;
...
function registerStickerTable(ctx) {
    if (ctx.model.tables?.interlude_sticker)
        return;
...
function registerSchedulePreplanTable(ctx) {
    if (ctx.model.tables?.interlude_schedule_preplan)
        return;
